...
audv.set = function (type) {
    ...
    if (type === "type") {
        window.wallpaperRegisterAudioListener((audio) => { this.audio = audio });
...
audv.run = function () {
    if (audv.opt.maintype == "strip" && this.maintmp.strip) { sel.visualizerBars('drawCanvas', this.audio) }
    else if (audv.opt.maintype == "round" && this.maintmp.round) { sel.visualizerCircle('drawCanvas', this.audio) }
}